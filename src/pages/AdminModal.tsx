...
const STATUS_CFG: Record<string, {
  label: string; dot: string; bg: string; border: string; text: string; pulse?: boolean;
}> = {
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (cfg) {
    return (
      <span style={{
        ...
        fontSize: 9, fontWeight: 800, color: cfg.text, letterSpacing: '0.07em', textTransform: 'uppercase',
      }}>
  ...
  const map: Record<string, string> = {
    ...
    PROCESSED: 'bg-emerald-900/30 text-emerald-400',
  };
  return (
...
function ErrorState({ text, onRetry }: { text: string; onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
      ...
      {onRetry && (
        <button onClick={onRetry} className="px-4 py-2 rounded-xl bg-slate-700 text-slate-300 text-xs font-bold hover:bg-slate-600 transition-colors">
          Try Again
        </button>
      )}
    </div>
...
const LEAGUE_SUGGESTIONS = [
  ...
  'World Cup','Euro 2024','Nations League','Africa Cup','SpeedBet Special',
  'SpeedBet Invitational','Friendly',
];
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {(['url', 'upload'] as const).map((m) => (
            <button key={m} type="button" onClick={() => setMode(m)} style={{
              ...
              color: mode === m ? '#63d2ff' : 'rgba(255,255,255,0.3)', cursor: 'pointer',
            }}>
              {m === 'upload' ? '📁 Upload' : '🔗 URL'}
            </button>
          ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          {mode === 'upload' ? (
            <div
              ...
              style={{ border: `1.5px dashed ${dragging ? '#63d2ff' : 'rgba(255,255,255,0.12)'}`, borderRadius: 8, padding: '10px 14px', cursor: 'pointer', background: dragging ? 'rgba(99,210,255,0.05)' : 'rgba(255,255,255,0.03)', textAlign: 'center' }}
            >
            ...
            <input type="url" value={value} onChange={(e) => onChange(e.target.value)} placeholder="https://example.com/logo.png"
              style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 12, outline: 'none', boxSizing: 'border-box' }}
        ...
        {value && (
          <button type="button" onClick={() => onChange('')} style={{ width: 24, height: 24, borderRadius: '50%', flexShrink: 0, background: 'rgba(255,71,87,0.15)', border: '1px solid rgba(255,71,87,0.3)', color: '#ff4757', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        )}
      </div>
    </div>
  );
...
function LeagueInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      ...
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} onFocus={() => setFocused(true)} placeholder="Premier League, La Liga…"
        style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: `1.5px solid ${focused ? 'rgba(99,210,255,0.5)' : 'rgba(255,255,255,0.1)'}`, borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
      />
      <AnimatePresence>
        {showDropdown && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.12 }}
            style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 100, background: '#13131f', border: '1.5px solid rgba(99,210,255,0.2)', borderRadius: 10, overflow: 'hidden', boxShadow: '0 16px 48px rgba(0,0,0,0.6)' }}
          >
            {(value.length === 0 ? LEAGUE_SUGGESTIONS.slice(0, 8) : filtered).map((l) => (
              <button key={l} type="button" onMouseDown={() => { onChange(l); setFocused(false); }}
                style={{ width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.75)', background: 'transparent', border: 'none', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}
...
const BLANK_FORM = {
  ...
  homeLogo: '', awayLogo: '', leagueLogo: '', kickoffDate: '', kickoffTime: '',
...
function CreateMatchModal({ onClose, onCreated }: { onClose: () => void; onCreated: (m: Match) => void }) {
  ...
  const create = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        homeTeam: form.homeTeam.trim(), awayTeam: form.awayTeam.trim(),
        league: form.league?.trim() || undefined, sport: form.sport || 'football',
        homeLogo: form.homeLogo?.trim() || undefined, awayLogo: form.awayLogo?.trim() || undefined,
        leagueLogo: form.leagueLogo?.trim() || undefined, status: form.status || 'SCHEDULED',
      ...
      if (res.success && res.data) {
        ...
        setError('Failed to create match — unexpected response.');
      }
    } catch (err: unknown) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div initial={{ opacity: 0, scale: 0.96, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 16 }} transition={{ type: 'spring', stiffness: 400, damping: 32 }}
          style={{ width: '100%', maxWidth: 560, background: 'linear-gradient(160deg, #0f0f1a 0%, #13131f 60%, #0d0d18 100%)', border: '1.5px solid rgba(99,210,255,0.15)', borderRadius: 18, boxShadow: '0 32px 80px rgba(0,0,0,0.9)', overflow: 'hidden', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}
        >
          <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                {[1, 2].map((s) => (
                  <div key={s} style={{ width: s === step ? 20 : 6, height: 6, borderRadius: 3, background: s === step ? '#63d2ff' : s < step ? 'rgba(99,210,255,0.4)' : 'rgba(255,255,255,0.1)', transition: 'all 0.3s' }} />
                ))}
              </div>
              <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {step === 1 ? (
                <motion.div key="s1" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.18 }} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    {[
                      { team: form.homeTeam, logo: form.homeLogo, color: '#63d2ff' },
                      { team: form.awayTeam, logo: form.awayLogo, color: '#ff4757' },
                      ...
                      acc.push(
                        <div key={i} style={{ textAlign: 'center', flex: 1 }}>
                          {item.logo
                            ? <img src={item.logo} alt="" style={{ width: 40, height: 40, objectFit: 'contain', margin: '0 auto 6px', display: 'block' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                            : <div style={{ width: 40, height: 40, borderRadius: '50%', margin: '0 auto 6px', background: `${item.color}1a`, border: `1.5px solid ${item.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: item.color }}>
                                {item.team ? item.team.slice(0, 3).toUpperCase() : '?'}
                              </div>
                          }
                          <div style={{ fontSize: 12, fontWeight: 700, color: item.team ? '#fff' : 'rgba(255,255,255,0.2)' }}>{item.team || (i === 0 ? 'Home' : 'Away')}</div>
                        </div>
                      );
                      return acc;
                    }, [])}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    {[['homeTeam', 'Home Team *', 'Man City'], ['awayTeam', 'Away Team *', 'Arsenal']].map(([k, label, ph]) => (
                      <div key={k}>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>{label}</div>
                        <input type="text" value={(form as Record<string, unknown>)[k] as string} onChange={(e) => set(k, e.target.value)} placeholder={ph}
                          style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                          onFocus={(e) => (e.target.style.borderColor = 'rgba(99,210,255,0.5)')}
                          onBlur={(e) => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')}
                        />
                      </div>
                ...
                <motion.div key="s2" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.18 }} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>Sport</div>
                    <select value={form.sport} onChange={(e) => set('sport', e.target.value)}
                      style={{ width: '100%', background: '#1a1a2e', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 13, outline: 'none', cursor: 'pointer', boxSizing: 'border-box', appearance: 'none' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    {[['kickoffDate', 'Kickoff Date', 'date'], ['kickoffTime', 'Kickoff Time', 'time']].map(([k, label, type]) => (
                      <div key={k}>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>{label}</div>
                        <input type={type} value={(form as Record<string, unknown>)[k] as string} onChange={(e) => set(k, e.target.value)}
                          style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 14px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box', colorScheme: 'dark' }}
                          onFocus={(e) => (e.target.style.borderColor = 'rgba(99,210,255,0.5)')}
                          onBlur={(e) => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')}
                        />
                      </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
                      {STATUS_OPTIONS_CREATE.map((s) => {
                        const cfg = STATUS_CFG[s];
                        const selected = form.status === s;
                        return (
                          <button key={s} type="button" onClick={() => set('status', s)} style={{ padding: '9px 12px', borderRadius: 8, cursor: 'pointer', border: `1.5px solid ${selected ? cfg.border : 'rgba(255,255,255,0.08)'}`, background: selected ? cfg.bg : 'transparent', color: selected ? cfg.text : 'rgba(255,255,255,0.35)', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6, transition: 'all 0.15s' }}>
                            <span style={{ width: 6, height: 6, borderRadius: '50%', background: selected ? cfg.dot : 'rgba(255,255,255,0.15)', flexShrink: 0 }} />
                            {cfg.label}
                          </button>
                        );
                      })}
                  ...
                  <button type="button" onClick={() => set('featured', !form.featured)} style={{ padding: '12px 16px', borderRadius: 10, cursor: 'pointer', border: `1.5px solid ${form.featured ? 'rgba(251,191,36,0.4)' : 'rgba(255,255,255,0.08)'}`, background: form.featured ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: form.featured ? '#fbbf24' : 'rgba(255,255,255,0.6)', marginBottom: 2 }}>⭐ Feature in hero carousel</div>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>Show this match on the homepage</div>
                    </div>
                    <div style={{ width: 40, height: 22, borderRadius: 11, background: form.featured ? '#fbbf24' : 'rgba(255,255,255,0.1)', position: 'relative', flexShrink: 0, transition: 'background 0.2s' }}>
                      <div style={{ position: 'absolute', top: 3, left: form.featured ? 21 : 3, width: 16, height: 16, borderRadius: '50%', background: '#fff', transition: 'left 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.3)' }} />
                    </div>
                  </button>
                </motion.div>
              )}
          ...
          <div style={{ padding: '14px 24px 20px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 10, flexShrink: 0 }}>
            {error ? (
              <div style={{ flex: 1, padding: '8px 12px', borderRadius: 8, background: 'rgba(255,71,87,0.1)', border: '1px solid rgba(255,71,87,0.3)', fontSize: 11, color: '#ff6b7a', fontWeight: 600 }}>
                {error}
                <button style={{ marginLeft: 8, textDecoration: 'underline', opacity: 0.7, background: 'none', border: 'none', color: 'inherit', cursor: 'pointer' }} onClick={() => setError('')}>Dismiss</button>
              </div>
            ) : (
              <>
                {step === 2 && (
                  <button onClick={() => setStep(1)} style={{ padding: '10px 20px', borderRadius: 9, border: '1.5px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>← Back</button>
                )}
                <button onClick={step === 1 ? () => { if (canNext) { setError(''); setStep(2); } else setError('Team names are required.'); } : create} disabled={saving}
                  style={{ flex: 1, padding: '12px 20px', borderRadius: 9, background: saving ? 'rgba(99,210,255,0.2)' : 'linear-gradient(135deg, #63d2ff, #3891ff)', border: 'none', color: saving ? 'rgba(255,255,255,0.5)' : '#fff', fontSize: 13, fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer', letterSpacing: '0.06em', textTransform: 'uppercase', boxShadow: saving ? 'none' : '0 4px 20px rgba(99,210,255,0.3)' }}>
...
function ScoreModal({ match, onClose, onSaved }: { match: Match; onClose: () => void; onSaved: (m: Match) => void }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = normalise<Match>(raw);
      if (res.success && res.data) {
        showToast(`Score updated: ${res.data.scoreHome}–${res.data.scoreAway}`, 'success');
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
      >
        <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
          style={{ width: '100%', maxWidth: 380, background: 'linear-gradient(160deg, #0f0f1a, #13131f)', border: '1.5px solid rgba(74,222,128,0.2)', borderRadius: 18, padding: 24, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4ade80', marginBottom: 4 }}>🟢 Live Score</div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: '20px 16px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              {[
                { team: match.homeTeam, borderColor: 'rgba(99,210,255,0.3)', value: home, onChange: setHome },
                { team: match.awayTeam, borderColor: 'rgba(255,71,87,0.3)', value: away, onChange: setAway },
              ].reduce<React.ReactNode[]>((acc, item, i) => {
                if (i === 1) acc.push(<div key="sep" style={{ fontSize: 24, color: 'rgba(255,255,255,0.2)', fontWeight: 900, flexShrink: 0 }}>–</div>);
                acc.push(
                  <div key={String(i)} style={{ flex: 1, textAlign: 'center' }}>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginBottom: 8, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{item.team}</div>
                    <input type="number" min={0} value={item.value} onChange={(e) => item.onChange(e.target.value)}
                      style={{ width: '100%', textAlign: 'center', fontSize: 36, fontWeight: 900, fontFamily: 'monospace', color: '#fff', background: 'rgba(255,255,255,0.06)', border: `2px solid ${item.borderColor}`, borderRadius: 10, padding: '8px 4px', outline: 'none', boxSizing: 'border-box' }}
                    />
                  </div>
                );
                return acc;
...
function StatusTransitionModal({ match, onClose, onSaved }: { match: Match; onClose: () => void; onSaved: (m: Match) => void }) {
  ...
  const transition = async (target: string) => {
    ...
    try {
      ...
      const res = normalise<Match>(raw);
      if (res.success && res.data) {
        showToast(`Status → ${STATUS_CFG[target]?.label ?? target}`, 'success');
        onSaved(res.data); onClose();
      }
    } catch (err: unknown) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
          style={{ width: '100%', maxWidth: 360, background: 'linear-gradient(160deg, #0f0f1a, #13131f)', border: `1.5px solid ${cfg.border}`, borderRadius: 18, padding: 24, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: cfg.text, marginBottom: 4 }}>Match Status</div>
              <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: '#fff' }}>{match.homeTeam} vs {match.awayTeam}</h3>
            </div>
            <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)', fontSize: 16, cursor: 'pointer' }}>×</button>
          </div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 99, background: cfg.bg, border: `1.5px solid ${cfg.border}`, marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: cfg.text, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{cfg.label}</span>
          </div>
          {next.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '20px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: 24, marginBottom: 8 }}>🏁</div>
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', margin: 0 }}>Match is finished — no further transitions.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>Move to →</div>
              {next.map((s) => {
                const tc = STATUS_CFG[s] ?? STATUS_CFG.SCHEDULED;
                return (
                  <button key={s} onClick={() => transition(s)} disabled={saving}
                    style={{ padding: '13px 16px', borderRadius: 10, cursor: saving ? 'not-allowed' : 'pointer', border: `1.5px solid ${tc.border}`, background: tc.bg, display: 'flex', alignItems: 'center', justifyContent: 'space-between', opacity: saving ? 0.5 : 1, transition: 'all 0.15s' }}
                    onMouseEnter={(e) => { if (!saving) (e.currentTarget as HTMLButtonElement).style.opacity = '0.8'; }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.opacity = saving ? '0.5' : '1'; }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: tc.dot, flexShrink: 0 }} />
                      <span style={{ fontSize: 13, fontWeight: 800, color: tc.text, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{tc.label}</span>
                    </div>
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{s === 'FINISHED' ? 'End match' : 'Continue →'}</span>
                  </button>
                );
              })}
            </div>
...
function MatchCard({ match, index, onScoreClick, onStatusClick, onHide }: {
  match: Match; index: number;
  onScoreClick: (m: Match) => void;
  onStatusClick: (m: Match) => void;
  ...
  const statusLabel = isHalfTime ? 'HT'
    : isFinished ? 'FT'
    : isLive ? `${minute}'`
    : cfg.label;
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 9px', borderRadius: 99, background: cfg.bg, border: `1px solid ${cfg.border}` }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 800, color: cfg.text, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{statusLabel}</span>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {[
          ...
          { name: match.awayTeam, logo: match.awayLogo, score: match.scoreAway, winning: hasScore && (match.scoreAway ?? 0) > (match.scoreHome ?? 0) },
      ...
      <div style={{ display: 'flex', gap: 7, marginTop: 'auto' }}>
        <button onClick={() => onScoreClick(match)} disabled={!isLive}
          style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: `1px solid ${isLive ? 'rgba(74,222,128,0.25)' : 'rgba(255,255,255,0.06)'}`, background: isLive ? 'rgba(74,222,128,0.12)' : 'rgba(255,255,255,0.04)', color: isLive ? '#4ade80' : 'rgba(255,255,255,0.2)', fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: isLive ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
          ⚽ Score
        </button>
        <button onClick={() => onStatusClick(match)} disabled={isFinished}
          style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: `1px solid ${isFinished ? 'rgba(255,255,255,0.04)' : 'rgba(99,210,255,0.2)'}`, background: isFinished ? 'rgba(255,255,255,0.02)' : 'rgba(99,210,255,0.1)', color: isFinished ? 'rgba(255,255,255,0.15)' : '#63d2ff', fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: isFinished ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}>
          ⚡ Status
        </button>
      </div>
    </motion.div>
    ...
    <div className="space-y-4">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: 12, overflowX: 'auto' }}>
        {MATCH_TABS.map((t) => {
          ...
          return (
            <button key={t} onClick={() => setFilter(t)} style={{ padding: '6px 14px', borderRadius: 8, border: `1.5px solid ${active ? 'rgba(99,210,255,0.4)' : 'rgba(255,255,255,0.07)'}`, background: active ? 'rgba(99,210,255,0.1)' : 'transparent', color: active ? '#63d2ff' : 'rgba(255,255,255,0.35)', fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
              {t === 'LIVE' && liveCount > 0 && <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#4ade80', flexShrink: 0 }} />}
...
// ─── Booking Code shared input style ─────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (bookingType === 'ADMIN_ONLY') {
          const raw = await adminMatches.list();
          const res = normalise<Match[]>(raw);
          fetchedMatches = (res.success && Array.isArray(res.data) ? res.data : []).map((m: any) => ({ ...m, _source: 'ADMIN' }));
        } else if (bookingType === 'MIXED') {
          const [adminRaw, feedRaw] = await Promise.all([adminMatches.list(), matchesApi.upcoming()]);
          const adminRes = normalise<Match[]>(adminRaw);
          const feedRes  = normalise<Match[]>(feedRaw);
          const adminMs = (adminRes.success && Array.isArray(adminRes.data) ? adminRes.data : []).map((m: any) => ({ ...m, _source: 'ADMIN' }));
          const feedMs  = (feedRes.success && Array.isArray(feedRes.data) ? feedRes.data : []).map((m: any) => ({ ...m, _source: 'EXTERNAL' }));
          const seen = new Set(adminMs.map((m: any) => m.id));
          fetchedMatches = [...adminMs, ...feedMs.filter((m: any) => !seen.has(m.id))];
        } else {
          const raw = await matchesApi.upcoming();
          const res = normalise<Match[]>(raw);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <BcField label="Market">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {MARKETS.map((m) => (
              <button key={m.value} onClick={() => { setMarket(m); setPick(''); }}
                style={{ padding: '8px 10px', borderRadius: 8, border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', textAlign: 'left', background: market.value === m.value ? typeCfg.accent : 'rgba(255,255,255,0.06)', color: market.value === m.value ? '#fff' : 'rgba(255,255,255,0.6)', transition: 'background .12s' }}>
                {m.label}
              </button>
            ))}
          </div>
  ...
  return (
    <BcModalShell title={`Code · ${code.code}`} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {((code as any).selections ?? []).map((s: any, i: number) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.04)' }}>
              <div style={{ width: 22, height: 22, borderRadius: 6, background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.4)', flexShrink: 0 }}>{i + 1}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.match ?? s.homeTeam ?? 'Unknown match'}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ padding: '1px 6px', borderRadius: 4, background: 'rgba(99,210,255,0.1)', fontWeight: 700, fontSize: 10, color: '#63d2ff' }}>{s.market}</span>
                  <span>{s.pick}</span>
                  {s.line != null && <span>· Line {s.line}</span>}
                </div>
              </div>
              <div style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 14, color: '#63d2ff', flexShrink: 0 }}>{(+s.odds).toFixed(2)}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={share} style={{ flex: 1, padding: '11px 0', borderRadius: 9, border: 'none', background: copied ? '#4ade80' : 'linear-gradient(135deg, #63d2ff, #3891ff)', color: copied ? '#000' : '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>
...
function BcStatCard({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700">
      <p className="text-xs text-slate-400 mb-1.5 uppercase tracking-wider font-bold">{label}</p>
      <p style={{ fontFamily: 'monospace', fontSize: 22, fontWeight: 700, color: accent ?? '#fff', lineHeight: 1 }}>{value}</p>
    </div>
  );
}
...
function BcCodeCard({ code, onView, onHide }: { code: BookingCode; onView: () => void; onHide: () => void }) {
  ...
  return (
    <div
      style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', transition: 'border-color .15s, box-shadow .15s' }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(99,210,255,0.35)'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.4)'; }}
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14, gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{code.label || '—'}</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5, flexShrink: 0 }}>
            ...
            {(code as any).bookingType && (code as any).bookingType !== 'STANDARD' && (
              <span style={{ padding: '2px 7px', borderRadius: 5, fontSize: 9, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', background: btCfg.accentBg, color: btCfg.accent }}>
                {(code as any).bookingType === 'ADMIN_ONLY' ? 'Admin Only' : 'Mixed'}
              </span>
            )}
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
          {[
            ...
            <div key={s.label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '8px 10px' }}>
              <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'rgba(255,255,255,0.4)', marginBottom: 2 }}>{s.label}</div>
              <div style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: s.accent ?? '#fff' }}>{s.value}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'rgba(255,255,255,0.35)', marginBottom: 14, flexWrap: 'wrap' }}>
          <span>🎟️ {selCount} selection{selCount !== 1 ? 's' : ''}</span>
          <span style={{ opacity: 0.4 }}>·</span>
          <span style={{ padding: '1px 7px', borderRadius: 4, fontSize: 10, fontWeight: 700, background: 'rgba(99,210,255,0.1)', color: '#63d2ff' }}>{(code as any).kind}</span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={copyCode} style={{ flex: 1, padding: '8px 0', borderRadius: 8, background: copied ? 'rgba(74,222,128,0.15)' : 'rgba(99,210,255,0.1)', border: `1px solid ${copied ? 'rgba(74,222,128,0.3)' : 'rgba(99,210,255,0.2)'}`, color: copied ? '#4ade80' : '#63d2ff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
            {copied ? '✓ Copied!' : '📋 Copy Code'}
          </button>
        </div>
      </div>
...
function BookingsSection() {
  ...
  const stats = useMemo(() => {
    const active   = visibleCodes.filter((c) => (c as any).status === 'active' && !isExpired((c as any).expiresAt ?? (c as any).expires_at)).length;
    const redeemed = visibleCodes.reduce((s, c) => s + +((c as any).redemptionCount ?? (c as any).redemption_count ?? 0), 0);
    const avgOdds  = visibleCodes.length ? (visibleCodes.reduce((s, c) => s + +((c as any).totalOdds ?? (c as any).total_odds ?? 0), 0) / visibleCodes.length).toFixed(2) : null;
    const avgStake = visibleCodes.length ? visibleCodes.reduce((s, c) => s + +(c.stake ?? 0), 0) / visibleCodes.length : null;
  ...
  return (
    <div className="space-y-5">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 700, color: 'rgba(255,255,255,0.3)', marginBottom: 2 }}>
            {loading ? 'Loading…' : `${visibleCodes.length} codes${hiddenCount > 0 ? ` (${hiddenCount} hidden)` : ''}`}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          <AnimatePresence>
            {visibleCodes.map((c, i) => (
              <motion.div key={(c as any).id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: Math.min(i * 0.04, 0.4), duration: 0.3 }}>
                <BcCodeCard code={c} onView={() => setViewCode(c)} onHide={() => hideCode((c as any).id)} />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
      ...
      <AnimatePresence>
        ...
        {(modalState === 'STANDARD' || modalState === 'ADMIN_ONLY' || modalState === 'MIXED') && (
          <BcCreateModal key="create" bookingType={modalState} onClose={() => setModalState(null)} onBack={() => setModalState('picker')} onCreate={handleCreate} />
        )}
...
function UpgradeChatPanel({ chat, isSuperAdmin, onClose, onCommissionSet }: { chat: AdminUpgradeChatDto; isSuperAdmin: boolean; onClose: () => void; onCommissionSet: () => void; }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const raw = await fn();
      const res = normalise<AdminUpgradeChatMessageDto>(raw);
      if (res.success) { setMessages((p) => [...p, res.data]); setMsgInput(''); }
    ...
    <div className="flex flex-col h-full">
      ...
      {chat.status !== 'CLOSED' && (
        <div className="px-4 py-3 border-t border-slate-700 bg-slate-800 shrink-0 flex gap-2">
          ...
          <button onClick={sendMessage} disabled={sending || !msgInput.trim()} className="p-2.5 bg-primary hover:bg-primary/80 disabled:opacity-50 text-white rounded-xl">
...
function DashboardSection({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isSuperAdmin) {
        const metricsRaw = await superAdmin.metrics();
        const metricsRes = normalise<Record<string, unknown>>(metricsRaw);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {kpis.map((kpi) => (
          <div key={kpi.label} className="bg-slate-800 rounded-2xl p-4 border border-slate-700">
            ...
            {loading ? <div className="h-7 bg-slate-700 rounded animate-pulse" /> : <p className="font-heading text-xl font-bold text-white">{kpi.value}</p>}
          </div>
        ))}
      </div>
      {affiliateStats && (
      ...
      <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700">
        ...
        {loading ? <div className="h-28 bg-slate-700 rounded animate-pulse" /> : (
          <div className="flex items-end gap-1 h-28">
            {(analytics?.dailyRevenue as number[] | undefined ?? [35, 55, 42, 68, 75, 60, 85, 90, 72, 95, 80, 88]).map((h, i) => {
              ...
              return <div key={i} className="flex-1 bg-primary/70 rounded-t hover:bg-primary transition-colors" style={{ height: `${pct}%` }} />;
            })}
          </div>
        )}
      </div>
...
function AffiliateSection({ userEmail }: { userEmail?: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [statsRaw, windowRaw, historyRaw, linksRaw, usersRaw] = await Promise.all([
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {statCards.map((card) => (
          <div key={card.label} style={{ background: '#fff', borderRadius: 14, padding: '16px 18px', boxShadow: '0 2px 12px rgba(0,0,0,0.25)' }}>
            ...
            {loading ? <div style={{ height: 28, background: '#f3f4f6', borderRadius: 6 }} /> : <p style={{ margin: 0, fontSize: 22, fontWeight: 800, color: card.label === 'TOTAL EARNED' ? '#16a34a' : '#111827', lineHeight: 1.1 }}>{card.value}</p>}
          </div>
        ))}
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: '18px 20px', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[1, 2].map(i => <div key={i} style={{ height: 52, background: 'rgba(255,255,255,0.04)', borderRadius: 10 }} />)}
          </div>
        ) : links.length === 0 ? (
              ...
              return (
                <div key={link.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {link.commissionPercent != null && <span style={{ fontSize: 10, fontWeight: 700, color: '#63d2ff' }}>{link.commissionPercent}% commission</span>}
                  </div>
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                    <button onClick={() => copyLink(link.id, url)} style={{ padding: 7, borderRadius: 7, background: isCopied ? 'rgba(74,222,128,0.15)' : 'rgba(255,255,255,0.06)', border: 'none', cursor: 'pointer', color: isCopied ? '#4ade80' : 'rgba(255,255,255,0.5)', display: 'flex' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {payoutHistory.map((pr) => (
              <div key={pr.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.04)' }}>
                <div><p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: '#fff' }}>{fmt(pr.amount, currency)}</p><p style={{ margin: 0, fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{fmtDate(pr.createdAt)}</p></div>
...
function WithdrawalsSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true); setFetchError(null);
    try {
      const raw = await withdrawals.getAllForAdmin(0, 20, statusFilter);
      const res = normalise<{ content: WithdrawalRequest[] }>(raw);
      if (res.success) setList(res.data?.content ?? (Array.isArray(res.data) ? res.data as unknown as WithdrawalRequest[] : []));
      else setFetchError('Failed to load withdrawals.');
  ...
  const approve = async (id: string) => {
    setProcessing(id);
    try {
      const raw = await withdrawals.approve(id, { note: 'Approved via admin panel' });
      const res = normalise<WithdrawalRequest>(raw);
  ...
  const reject = async (id: string) => {
    setProcessing(id);
    try {
      const raw = await withdrawals.reject(id, { note: 'Rejected via admin panel' });
      const res = normalise<WithdrawalRequest>(raw);
            ...
            {list.map((w) => (
              <div key={w.id} className="bg-slate-800 rounded-2xl p-4 border border-slate-700 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {w.status === 'PENDING' && (
                    <>
                      <button disabled={processing === w.id} onClick={() => approve(w.id)} className="p-1.5 rounded-lg bg-emerald-900/30 text-emerald-400 hover:bg-emerald-900/50 disabled:opacity-50">{processing === w.id ? <Spinner /> : <CheckIcon fontSize="small" />}</button>
                      <button disabled={processing === w.id} onClick={() => reject(w.id)} className="p-1.5 rounded-lg bg-red-900/30 text-red-400 hover:bg-red-900/50 disabled:opacity-50"><BlockIcon fontSize="small" /></button>
                    </>
                  )}
                </div>
              </div>
    ...
    <div className="fixed inset-0 z-[70] bg-slate-900 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="md:hidden absolute top-[52px] left-0 right-0 flex overflow-x-auto gap-1.5 px-3 py-2 border-b border-slate-700 bg-slate-900 z-10 shrink-0" style={{ scrollbarWidth: 'none' }}>
          {visibleSections.map((section) => (
            <button key={section.key} onClick={() => setActiveSection(section.key)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap min-h-[36px] transition-colors ${activeSection === section.key ? 'bg-primary text-white' : 'bg-slate-700 text-slate-300'}`}>
              {section.icon}
              {section.label}
            </button>
          ))}
