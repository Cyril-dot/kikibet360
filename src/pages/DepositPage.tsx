...
// 3 tab steps. Inside step 2 there are 3 sub-steps:
//   SMS    → user enters OTP code sent by MTN via SMS
//   WAIT   → user approves USSD prompt on phone
//   VERIFY → we check if payment went through
...
export default function DepositPage() {
  ...
  // Countdown — only during USSD wait
  useEffect(() => {
  ...
  const handleInit = async () => {
    ...
    try {
      const data = await post("/api/wallet/deposit/moolre/init", {
        amount: amt, phone: phone.trim(), network,
      });
      setRef(data?.data?.externalref || "");
      const isActionRequired = data?.data?.actionRequired === true;
      setActionRequired(isActionRequired);
      // MTN actionRequired=true → user must enter SMS code first
      // All other networks → go straight to USSD wait screen
  ...
  // ── Step 2a: Submit SMS OTP code → triggers USSD push ──────────────────────
  const handleSmsSubmit = async () => {
    ...
    try {
      ...
      // Code accepted — Moolre now pushes the USSD prompt to the user's phone
      setSmsCode("");
      setError("");
      setSub(SUB.WAIT);
    } catch (e) {
  ...
  const S = {
    ...
    body:     { padding:"20px" },
    err:      { background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:"8px", padding:"10px 12px", color:"#f87171", fontSize:"13px", marginBottom:"14px" },
  ...
  const tab = (active) => ({
    ...
    ...(active ? { background:"#1a3a5c", color:"#fff", borderBottom:"2px solid #4a9eff" }
      ...
      <div style={S.card}>
        ...
        <div style={S.body}>
          ...
          {step === STEP.DETAILS && (<>
            ...
            <div style={{ marginBottom:"18px" }}>
              ...
              <div style={{ display:"flex", gap:"8px" }}>
                ...
                <input type="tel" placeholder="0244123456" value={phone} onChange={e => setPhone(e.target.value)} maxLength={10} style={S.input} />
              </div>
            </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                {NETWORKS.map(n => (
                  <button key={n.id} onClick={() => setNetwork(n.id)}
                    style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background: network===n.id ? "rgba(74,158,255,0.08)" : "rgba(255,255,255,0.03)", border:`1px solid ${network===n.id ? "rgba(74,158,255,0.35)" : "rgba(255,255,255,0.07)"}`, borderRadius:"10px", padding:"12px 14px", cursor:"pointer" }}>
            ...
            <div style={{ ...S.info, marginBottom:"18px" }}>
              📲 A prompt will be sent to your MoMo phone. Approve it within 2 minutes to complete your deposit.
            </div>
          ...
          {step === STEP.APPROVE && (<>
            ...
            {/* ── SUB: SMS — enter the code Moolre/MTN sent via SMS ── */}
            {sub === SUB.SMS && (<>
              {/* Progress indicator */}
              <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"20px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"6px", flex:1 }}>
                  {[
                    { n:"1", label:"Enter SMS Code", active:true  },
                    { n:"2", label:"Approve USSD",   active:false },
                    { n:"3", label:"Done",            active:false },
                  ].map((s, i) => (
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:"6px", flex: i<2 ? 1 : "none" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:"5px" }}>
                        <div style={{ width:"22px", height:"22px", borderRadius:"50%", background: s.active ? "#2563eb" : "rgba(255,255,255,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"10px", fontWeight:800, color: s.active ? "#fff" : "#445566", flexShrink:0 }}>{s.n}</div>
                        <span style={{ fontSize:"11px", fontWeight:600, color: s.active ? "#aaccff" : "#445566", whiteSpace:"nowrap" }}>{s.label}</span>
                      </div>
                      {i < 2 && <div style={{ flex:1, height:"1px", background:"rgba(255,255,255,0.07)", minWidth:"12px" }} />}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:"8px", padding:"10px 12px", color:"#445566", fontSize:"12px", lineHeight:"1.5" }}>
                ...
                <span onClick={restart} style={{ color:"#4a9eff", cursor:"pointer", textDecoration:"underline" }}>start over</span>
                {" "}to resend.
              </div>
            </>)}
            ...
            {sub === SUB.WAIT && (<>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"20px" }}>
                {[
                  { n:"1", label:"SMS Code",     done: actionRequired, active: false },
                  { n:"2", label:"Approve USSD", done: false,          active: true  },
                  { n:"3", label:"Done",         done: false,          active: false },
                ].filter((s, i) => actionRequired || i !== 0).map((s, i, arr) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:"6px", flex: i < arr.length-1 ? 1 : "none" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:"5px" }}>
                      <div style={{ width:"22px", height:"22px", borderRadius:"50%", background: s.done ? "rgba(34,197,94,0.3)" : s.active ? "#2563eb" : "rgba(255,255,255,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"10px", fontWeight:800, color: s.done||s.active ? "#fff" : "#445566", flexShrink:0 }}>
                        {s.done ? "✓" : s.n}
                      </div>
                      <span style={{ fontSize:"11px", fontWeight:600, color: s.active ? "#aaccff" : s.done ? "#4ade80" : "#445566", whiteSpace:"nowrap" }}>{s.label}</span>
                    </div>
                    {i < arr.length-1 && <div style={{ flex:1, height:"1px", background:"rgba(255,255,255,0.07)", minWidth:"12px" }} />}
                  </div>
                ))}
              </div>
              ...
              <div style={{ background:"rgba(255,204,0,0.06)", border:"1px solid rgba(255,204,0,0.18)", borderRadius:"12px", padding:"18px", marginBottom:"14px", textAlign:"center" }}>
                ...
                <div style={{ color:"#998844", fontSize:"12px", lineHeight:"1.7" }}>
                  ...
                  <strong style={{ color:"#ccaa44" }}>{phone}</strong>.
                  <br />
                  Approve the <strong style={{ color:"#ffcc66" }}>GH₵{parseFloat(amount).toFixed(2)}</strong> payment
                  on your {networkLabel} to continue.
                </div>
                {countdown > 0
                  ? <div style={{ marginTop:"10px", fontSize:"12px", color:"#665522" }}>
                      Expires in <strong style={{ color:"#ffcc00" }}>{fmt(countdown)}</strong>
                    </div>
          ...
          {step === STEP.DONE && (<>
            ...
            <button onClick={() => { setStep(STEP.DETAILS); setAmount(""); setPhone(""); setNetwork("MTN"); setError(""); setInfo(""); setRef(""); setSmsCode(""); setSub(SUB.WAIT); }} style={S.btnSec}>
