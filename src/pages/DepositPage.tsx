...
import { useNavigate } from "react-router-dom"; // adjust to your router if needed
...
// SMS  = enter SMS code sent by MTN (only when actionRequired=true)
// WAIT = waiting for user to approve USSD prompt on phone
// VERIFY = check if payment went through
...
export default function DepositPage() {
  ...
  // Countdown timer — only active while waiting for USSD approval
  useEffect(() => {
  ...
  const handleInit = async () => {
    ...
    try {
      ...
      if (!res.ok)
        return setError(
          data?.message || data?.error || "Payment initiation failed. Please try again."
        );
      setRef(data?.data?.externalref || "");
      const isActionRequired = data?.data?.actionRequired === true;
      setActionRequired(isActionRequired);
      // If MTN requires SMS code verification, go to SMS sub-step first
      // Otherwise go straight to WAIT (USSD prompt)
  ...
  // ── Step 2a: Submit SMS code (MTN actionRequired flow) ──────────────────────
  const handleSmsSubmit = async () => {
    setError("");
    if (!smsCode.trim()) return setError("Please enter the SMS code.");
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/wallet/deposit/moolre/otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${tok()}`,
        },
        body: JSON.stringify({ externalref: externalRef, otp: smsCode.trim() }),
      });
      const data = await res.json();
      if (!res.ok)
        return setError(
          data?.message || "SMS code verification failed. Please check the code and try again."
        );
      // Code accepted — now wait for USSD prompt approval
      setSmsCode("");
      setSub(SUB.WAIT);
    } catch {
      setError("Network error. Please try again.");
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        return setError(data?.message || "Verification failed. Please try again.");
      const r = data?.data;
      if (r?.credited) {
        setStep(STEP.DONE);
      } else if (r?.txstatus === 0) {
        setInfo(
          "Payment still pending — please approve the prompt on your phone, then verify again."
        );
      } else if (r?.txstatus === 2) {
        setError("Payment was cancelled or failed. Please start a new deposit.");
      } else {
        setInfo(r?.message || "Status unclear. Try verifying again in a moment.");
      }
    } catch {
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "12px", color: "#556677", marginTop: "2px" }}>
            Mobile Money · GHS
          </div>
        </div>
        ...
        <div style={{ padding: "20px" }}>
          ...
          {step === STEP.DETAILS && (
            <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {NETWORKS.map(n => (
                    ...
                    >
                      <span
                        style={{
                          color: network === n.id ? "#cce0ff" : "#889aaa",
                          fontSize: "14px",
                          fontWeight: 600,
                        }}
                      >
                      ...
                      {network === n.id && (
                        <span style={{ color: "#4a9eff", fontSize: "13px", fontWeight: 700 }}>
                          ✓
                        </span>
                      )}
                    </button>
              ...
              >
                📲 A USSD prompt will be sent to your MoMo phone. Approve it within 2
                minutes to complete your deposit.
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  borderRadius: "10px",
                  color: loading || !amount || !phone ? "#334466" : "#fff",
                  fontSize: "15px",
                  fontWeight: 700,
                  cursor: loading || !amount || !phone ? "not-allowed" : "pointer",
                  transition: "all 0.2s",
                }}
              >
                {loading
                  ? "Initiating…"
                  : `Send USSD Prompt · GH₵${parseFloat(amount) || "0.00"}`}
              </button>
          ...
          {step === STEP.APPROVE && (
            <>
              ...
              {/* ── SUB: SMS — enter code sent by MTN ── */}
              {sub === SUB.SMS && (
                <>
                  <div style={{ textAlign: "center", marginBottom: "20px" }}>
                    ...
                    <div style={{ color: "#556677", fontSize: "12px", lineHeight: "1.6" }}>
                      ...
                      <br />
                      Enter it below to trigger the USSD approval prompt.
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    ...
                    >
                      Check your SMS inbox for a code from MTN MoMo
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: "15px",
                      background:
                        loading || !smsCode.trim()
                          ? "rgba(74,158,255,0.1)"
                          : "linear-gradient(135deg,#2563eb,#1d4ed8)",
                      border: "none",
                      borderRadius: "10px",
                      color: loading || !smsCode.trim() ? "#334466" : "#fff",
                      fontSize: "15px",
                      fontWeight: 700,
                      cursor: loading || !smsCode.trim() ? "not-allowed" : "pointer",
                      marginBottom: "10px",
                      transition: "all 0.2s",
                    }}
                  >
              ...
              {/* ── SUB: WAIT — waiting for USSD approval ── */}
              {sub === SUB.WAIT && (
                <>
                  <div
                    style={{
                  ...
                  >
                    ...
                    <div
                      style={{ color: "#998844", fontSize: "12px", lineHeight: "1.6" }}
                    >
                      A USSD prompt has been sent to{" "}
                      <strong style={{ color: "#ccaa44" }}>{phone}</strong>.
                      <br />
                      Approve the{" "}
                      <strong>GH₵{parseFloat(amount).toFixed(2)}</strong> payment on your{" "}
                      {networkLabel} to continue.
                    </div>
                    {countdown > 0 ? (
                      ...
                      <div
                        style={{ marginTop: "10px", fontSize: "12px", color: "#994433" }}
                      >
              ...
              >
                {[
                  ...
                  >
                    ...
                    <span style={{ color: "#aabbcc", fontSize: "12px", fontWeight: 600 }}>
                      {v}
                    </span>
                  </div>
                ))}
